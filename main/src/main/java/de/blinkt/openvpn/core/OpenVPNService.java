...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public void challengeResponse(String response) throws RemoteException {
        if (mManagement != null) {
            String b64response = Base64.encodeToString(
    ...
    private void jbNotificationExtras(
        ...
        try {
            if (priority != 0) {
                ...
                Method setUsesChronometer = nbuilder.getClass().getMethod(
                        "setUsesChronometer",
                        boolean.class
                );
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        ...
        if (mDeviceStateReceiver == null || !mDeviceStateReceiver.isUserPaused()) {
            ...
            PendingIntent pauseVPNPending = PendingIntent.getService(
                    this,
                    0,
                    pauseVPN,
                    PendingIntent.FLAG_IMMUTABLE
            ...
            PendingIntent resumeVPNPending = PendingIntent.getService(
                    this,
                    0,
                    pauseVPN,
                    PendingIntent.FLAG_IMMUTABLE
    ...
    PendingIntent getGraphPendingIntent() {
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
        PendingIntent startLW = PendingIntent.getActivity(
                this,
                0,
                intent,
                PendingIntent.FLAG_IMMUTABLE
    ...
    private VpnProfile fetchVPNProfile(Intent intent) {
        ...
        VpnStatus.logDebug(String.format(
                "Fetched VPN profile (%s) triggered by %s",
                name,
                startReason
        ));
        return vpnProfile;
    ...
    private boolean checkVPNPermission(VpnProfile startprofile) {
        ...
        showNotification(
                getString(R.string.permission_requested),
                "",
    ...
    private void startOpenVPN(Intent intent, int startId) {
        ...
        boolean noReplaceRequested = (intent != null) && intent.getBooleanExtra(
                EXTRA_DO_NOT_REPLACE_RUNNING_VPN,
                false
        );
    ...
    private OpenVPNManagement instantiateOpenVPN3Core() {
        try {
            ...
            return (OpenVPNManagement) cl.getConstructor(
    ...
    private static String getTunConfigString(TunConfig tc) {
        ...
        cfg += "routes: " + TextUtils.join("|", tc.mRoutes.getNetworks(true)) + TextUtils.join(
                "|",
                tc.mRoutesv6.getNetworks(true)
        );
        cfg += "excl. routes:" + TextUtils.join(
                "|",
                tc.mRoutes.getNetworks(false)
        ) + TextUtils.join("|", tc.mRoutesv6.getNetworks(false));
    ...
    private ParcelFileDescriptor openTun(TunConfig tc) {
        ...
        if (mProfile == null) {
            VpnStatus.logError(
                    "OpenVPN tries to open a VPN descriptor with mProfile==null, please report this bug with log!");
            return null;
        }
        ...
        VpnStatus.logInfo(
                ...
                TextUtils.join(", ", tc.mRoutes.getNetworks(true)),
                TextUtils.join(", ", tc.mRoutesv6.getNetworks(true))
        );
        VpnStatus.logInfo(
                R.string.routes_info_excl,
                TextUtils.join(", ", tc.mRoutes.getNetworks(false)),
                TextUtils.join(", ", tc.mRoutesv6.getNetworks(false))
        );
        if (tc.mProxyInfo != null) {
            VpnStatus.logInfo(
                    R.string.proxy_info,
                    tc.mProxyInfo.getHost(),
    ...
    private void installRoutesPostiveOnly(
            ...
            Collection<IpAddress> positiveIPv6Routes
    ) {
        ...
        switch (method) {
            ...
            case "CR_TEXT":
                ...
                intent.setComponent(new ComponentName(
                        this,
                        getPackageName() + ".activities.CredentialsPopup"
                ));
